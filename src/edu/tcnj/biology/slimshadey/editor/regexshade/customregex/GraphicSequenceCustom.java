...
import shahavi.bio471.graphicsequence.ColorableLabel;
 ...
 */
public class GraphicSequenceCustom extends Group {

    private GraphicSequenceViewerCustom gsvorig;

    private ArrayList<GraphicSequenceMatchCustom> graphicList;
    private List<GraphicHitCustom> match;
     ...
     * @param originator
     */
    public GraphicSequenceCustom(double width, double graphicSeparator, Color defCol,
            int index, String FASTA, List<GraphicHitCustom> match, String name,
            GraphicSequenceViewerCustom originator) {
        super();
        this.FASTA = FASTA;
    ...
    private void createGraphics(String sequence, List<GraphicHitCustom> match) {
        ...
        for (int k = 0; k < match.size(); k++) {
            GraphicHitCustom gh = match.get(k);
            Color c = gh.c;
            List<OrderedPair> opl = gh.h.HIT_LIST;
            System.out.println(gh.h.PATTERNSTR + " " + gh.c.toString() + " asdf");
            ...
            for (int j = 0; j < opl.size(); j++) {
                ...
                GraphicSequenceMatchCustom attempt = new GraphicSequenceMatchCustom(x, y, w, h, c, gh.h.PATTERNSTR, null, null, -1, -1);
    ...
    private void update(double x) {
        ...
        for (GraphicHitCustom gh : match) {
            CustomRegexHit h = gh.h;
    ...
    private void docPopup(GraphicSequenceMatchCustom gsm, GraphicHitCustom origin) {

        
        String doc = origin.h.PATTERNSTR;
...
popup.getIcons().add(new Image(getClass().getClassLoader().getResourceAsStream("resources/icons/icon3.png")));
        popup.setTitle("Custom pattern: " + doc);
    ...
    public void regularizeColorsPopup(GraphicHitCustom gh, GraphicSequenceMatchCustom gsm) {
        Stage st = new Stage();
st.getIcons().add(new Image(getClass().getClassLoader().getResourceAsStream("resources/icons/icon3.png")));
    ...
    public void applyAll_externalCommand(GraphicHitCustom gh, GraphicSequenceMatchCustom gsm) {
        for (GraphicHitCustom ghit : match) {
            if (gh.h.PATTERNSTR.equals(ghit.h.PATTERNSTR)) {
                ghit.updateColor(gsm.getColor());
            }
        }
    }

    public void allowAll_externalCommand(GraphicHitCustom gh, boolean allow) {
        for (GraphicHitCustom ghit : match) {
            if (gh.h.PATTERNSTR.equals(ghit.h.PATTERNSTR)) {
                ghit.allowAll(allow);
            }
    ...
    public GraphicHitCustom getGraphicHit(int k) {
        return match.get(k);
    }
}
